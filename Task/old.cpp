...
struct CameraData {
    float3 position;
    float yaw;   // угол по горизонтали (в градусах)
    float pitch; // угол по вертикали (в градусах)
    float3 up;
    float fov_y;
    float aspect_ratio;
};
...
// Абстрактный базовый класс для объектов сцены
class SceneObject {
public:
...
// Класс для плоскости (A1)
class Plane : public SceneObject {
public:
    float a, b, c, d; // Коэффициенты уравнения плоскости ax + by + cz + d = 0
    float3 normal;
...
struct BVHNode {
    AABB bounds;
    std::vector<int> triangle_indices; // Для листовых узлов
...
public:
    ...
    bool use_bvh = false; // Переключатель между A2 (наивный) и A3 (BVH)
  ...
  void load_from_obj(const std::string& path) {
    ...
    // Масштабируем и центрируем меш
    float scale = 1.0f / max_size; // Меш в [-0.5, 0.5]^3
    for (const auto& v : loaded_mesh.vPos4f) {
        float3 pos = float3(v.x, v.y, v.z);
...
private:
    ...
    std::unique_ptr<BVHNode> build_bvh_recursive(std::vector<int>& tri_indices, size_t start, size_t end) {
        ...
        // Выбираем ось с максимальной протяженностью
        float3 extent = centroid_bounds.max - centroid_bounds.min;
        int axis = (extent.x > extent.y && extent.x > extent.z) ? 0 : (extent.y > extent.z ? 1 : 2);

        // Сортируем по центроидам вдоль оси
...
float3 trace(const Scene& scene, const Ray& ray, int depth, int max_depth) {
    ...
    if (scene.intersect(ray, isect)) {
        ...
        } else { // Диффузное освещение по Ламберту
...
// -----------------------------------------------------------------
// Часть B1: Отрисовка модели, представленной SDFGrid с использованием sphere tracing
// -----------------------------------------------------------------

// Функция для трёхлинейной интерполяции SDF в точке p, где p ∈ [-1,1]^3
float sdf_trilinear(const SdfGrid &grid, const float3 &p)
{
    // Преобразуем координаты из [-1,1] в [0, size-1]
    ...
    // Интерполяция по x, затем по y и z
...
// Функция вычисления нормали в точке p методом конечных разностей
...
bool sphere_trace(const SdfGrid &grid, const Ray &ray, float &t_hit, int max_steps = 100, float eps = 0.001f)
{
    ...
    for (int i = 0; i < max_steps; i++)
    {
        ...
        // Если вышли за пределы куба [-1,1]^3, считаем, что пересечения нет
...
// Отрисовка SDF модели (часть B1) с использованием sphere tracing и освещения по Ламберту
void draw_frame_sdf(const AppData &app_data, const Camera &camera, const SdfGrid &grid, std::vector<uint32_t> &pixels)
{
    int width = app_data.width;
    int height = app_data.height;
    // Параметры света (можно расширять)
...
// -----------------------------------------------------------------
// Часть B2: Построение SDF сетки по мешу
// -----------------------------------------------------------------

// Функция вычисления ближайшей точки на треугольнике (a,b,c) к точке p
...
// Вычисление SDF для точки p по мешу (по заданной формуле)
...
// Построение регулярной SDF сетки по мешу (Часть B2)
// Резолюция задаётся равномерно по всем осям, область [-1,1]^3
void build_sdf_grid_from_mesh(const Mesh &mesh, SdfGrid &grid, int resolution)
{
    ...
    for (int z = 0; z < resolution; z++)
    {
        for (int y = 0; y < resolution; y++)
        {
            ...
            {
                // Преобразуем координаты узла в пространство [-1,1]^3
...
// -----------------------------------------------------------------
// Часть B3: Модификация SDF сетки (вырезание сферы)
// -----------------------------------------------------------------

// Функция модификации SDF сетки: вырезание сферы с центром sphere_center и радиусом sphere_radius.
// При булевой операции разности: SDF_new = max( SDF_model, -SDF_sphere )
...
// Вычисление SDF в точке p с использованием октодерева
float sdf_octree_at_point(const SdfOctree &octree, const float3 &p, int max_depth) {
    ...
    float3 local_p = (p - node_min) / node_size; // Используем текущий размер узла
    float fx = local_p.x;
    float fy = local_p.y;
    float fz = local_p.z;
...
bool sphere_trace_octree(const SdfOctree &octree, const Ray &ray, float &t_hit, int max_depth,
    ...
    for (int i = 0; i < max_steps; i++) {
        ...
        if (d < eps) { // Изменено с fabs(d) < eps
            return true;
        }
        t_hit += d;
...
// Отрисовка SDF Octree с использованием sphere tracing
...
// Вспомогательная структура для построения узлов
...
void modify_sdf_octree(SdfOctree &octree, const float3 &sphere_center, float sphere_radius, int max_depth) {
    ...
    while (!stack.empty()) {
        ...
        // Модификация SDF с вычитанием сферы
        for (int i = 0; i < 8; i++) {
            float3 corner = current.min;
            ...
            node.values[i] = node.values[i] - sphere_sdf_val; // Вычитание вместо max
        }
        ...
        if (!node_has_surface(node) || current_depth >= max_depth) {
            // Помечаем дочерние узлы на удаление
...
int main(int argc, char **args)
{
  ...
  // Загружаем SDF-сетку (если ранее была построена)
  load_sdf_grid(app_data.loaded_grid, "example_grid.grid");
  // Добавляем SDF Octree
  SdfOctree octree;
  int octree_depth = 8; // Глубина октодерева по умолчанию
  ...
  if (!renderer)
  {
    std::cerr << "Не удалось создать рендерер! SDL_Error: " << SDL_GetError() << std::endl;
  ...
  if (!texture)
  {
    std::cerr << "Не удалось создать текстуру! SDL_Error: " << SDL_GetError() << std::endl;
  ...
  // Параметры для режима B3 (вырезаемую сферу)
  float3 sphereCenter = float3(0, 0.5f, 0);
  float sphereRadius = 0.5f;
  // Если заданы параметры через командную строку, их можно обработать здесь
  ...
  while (running)
  {
      ...
      else if (ev.type == SDL_KEYDOWN)
      {
        switch (ev.key.keysym.sym)
        {
          ...
          case SDLK_4:
            renderMode = 4;
            break;
          // Управление сферой для модификации SDF (B3)
          case SDLK_EQUALS:
            sphereRadius += 0.05f;
            break;
          case SDLK_MINUS:
    ...
    // В зависимости от renderMode выбираем режим отрисовки:
    if (renderMode == 1)
    ...
    else if (renderMode == 3)
    {
      ...
      Mesh mesh;
      mesh.load_from_obj("cube.obj"); // или другой объект, который удовлетворяет условиям
      mesh.build_bvh();
      SdfGrid generatedGrid;
      int resolution = 128; // можно изменить до 256
